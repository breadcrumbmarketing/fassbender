...
/* Min/Max Quantity */
if(get_theme_mod('medibazar_min_max_quantity',0) == 1){
...
/* Recently Viewed */
if(get_theme_mod('medibazar_recently_viewed_products',0) == 1){
	require_once( __DIR__ . '/recently-viewed/recently-viewed.php' );
}

/* Product360 View */
if(get_theme_mod('medibazar_shop_single_product360',0) == 1){
	require_once( __DIR__ . '/product360/product360.php' );
}

/* Single Products Navigation */
if(get_theme_mod('medibazar_products_navigation',0) == 1){
	require_once( __DIR__ . '/single-products-navigation/single-products-navigation.php' );
}

/* Min Amount */
if(get_theme_mod('medibazar_min_order_amount_toggle',0) == 1){
	require_once( __DIR__ . '/min-amount/min-amount.php' );
}

/* Buy Now Single */
if(get_theme_mod('medibazar_shop_single_buy_now',0) == 1 || medibazar_ft() == 'buynow'){
	require_once( __DIR__ . '/buy-now/buy-now.php' );
}

/* Sticky Single Cart */
if(get_theme_mod('medibazar_single_sticky_cart',0) == 1){
	require_once( __DIR__ . '/sticky-single-cart/sticky-single-cart.php' );
}

/* Notice Ajax */
if(get_theme_mod('medibazar_shop_notice_ajax_addtocart',0) == 1 || medibazar_ft() == 'notice_ajax'){
...
*************************************************/
if(get_theme_mod('medibazar_shop_single_review_tab_move', 0) == 1 || medibazar_ft() == 'movereviewtab'){
	add_action( 'woocommerce_after_single_product_summary', 'medibazar_move_review_tab', 5 );
	function medibazar_move_review_tab(){
	  comments_template();
	}

	add_filter( 'woocommerce_product_tabs', 'medibazar_remove_product_tabs', 98 );
	function medibazar_remove_product_tabs( $tabs ){
		unset( $tabs['reviews'] );
...
/*************************************************
## Medibazar Order on WhatsApp
*************************************************/
if(get_theme_mod('medibazar_shop_single_orderonwhatsapp', 0) == 1 || medibazar_ft() == 'orderonwhatsapp'){
	add_action('woocommerce_single_product_summary', 'medibazar_order_on_whatsapp', 30);
	function medibazar_order_on_whatsapp(){
		$number = get_theme_mod('medibazar_shop_single_whatsapp_number');
		
		echo '<div class="orderon-whatsapp">';
		echo '<a href="https://wa.me/'.esc_attr($number).'?text='.get_the_title().'" class="button c-btn">'.esc_html__('Order on WhatsApp','medibazar-core').'</a>';
		echo '</div>';
	}
}
...
*************************************************/
function medibazar_product_stock_progress_bar(){
	global $product;
	...
	if($managestock) {
		...
		$output .= '<div class="sold">'.esc_html__('Sold:','medibazar-core').' <strong>'.esc_html($total_sales).'</strong></div>';
		$output .= '<div class="available">'.esc_html__('Available:','medibazar-core').' <strong>'.esc_html($stock_quantity).'</strong></div>';
		$output .= '</div>';
...
*************************************************/
function medibazar_product_time_countdown(){		
	$id = get_the_ID();
	...
	if($sale_price_dates_to && $sale_price_dates_from <= date("Y/m/d")){
		wp_enqueue_script( 'medibazar-counter');
		...
		$output .= '<div class="expired-text">'.esc_html__('Remains until the end of the offer','medibazar-core').'</div>';
