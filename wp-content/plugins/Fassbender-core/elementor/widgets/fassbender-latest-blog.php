...
class Fassbender_Latest_Blog_Widget extends Widget_Base {
    use Fassbender_Helper;

    public function get_name() {
        return 'Fassbender-latest-blog';
    }
	...
	protected function register_controls() {
       ...
       $this->add_control( 'excerpt_size',
            [
                'label' => esc_html__( 'Excerpt Size', 'Fassbender-core' ),
                'type' => Controls_Manager::NUMBER,
                'min' => 0,
                'max' => 100,
                'default' => 15
        ...
        $this->add_control( 'category_filter',
            [
                ...
                'multiple' => true,
                'options' => $this->Fassbender_get_categories(),
        ...
        $this->add_control( 'post_filter',
            [
                ...
                'type' => Controls_Manager::SELECT2,
                'multiple' => true,
                'options' => $this->Fassbender_get_posts(),
	...
	protected function render() {
		...
		if ( $loop->have_posts() ) {
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				$imageresize = Fassbender_resize( $image_src, 260, 170, true, true, true );
				...
				$output .= '<p>'.Fassbender_limit_words(get_the_excerpt(), $settings['excerpt_size']).' </p>';
				$output .= '<div class="b-button gray-b-button">';
				$output .= '<a href="'.get_permalink().'">'.esc_html__('read more','Fassbender-core').' <i class="far fa-plus"></i></a>';
				$output .= '</div>';
				$output .= '</div>';
				$output .= '</div>';
				$output .= '</div>';
