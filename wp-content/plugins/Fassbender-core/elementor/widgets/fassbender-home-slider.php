...
class Fassbender_Home_Slider_Widget extends \Elementor\Widget_Base {

    public function get_name() {
        return 'Fassbender-home-slider';
    }
	...
	protected function render() {
		...
		if ( $settings['slider_items'] ) {
			...
			foreach ( $settings['slider_items'] as $item ) {
				...
				echo '<h2>'.Fassbender_sanitize_data($item['sale_tag']).'</h2>';
				echo '</div>';
				}
				echo '<div class="banner-content">';
				echo '<span>'.esc_html($item['slider_subtitle']).'</span>';
				echo '<h2>'.esc_html($item['slider_title']).'</h2>';
				echo '<p>'.Fassbender_sanitize_data($item['slider_desc']).'</p>';
