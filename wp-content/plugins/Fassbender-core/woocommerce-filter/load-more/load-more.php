...
if(get_theme_mod('Fassbender_paginate_type') == 'loadmore' || Fassbender_ft() == 'load-more'){
	function Fassbender_load_more_button(){
		echo '<nav class="woocommerce-pagination klb-load-more">
                <ul class="page-numbers">
					<div class="c-btn button light">'.esc_html__('Load More','Fassbender-core').'</div>
				</ul>
			  </nav>';
	}

	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'Fassbender_load_more_button', 10 );
}
...
if(get_theme_mod('Fassbender_paginate_type') == 'infinite' || Fassbender_ft() == 'infinite'){
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
}
...
if(get_theme_mod('Fassbender_ajax_on_shop',0) == '1'){
	add_action('woocommerce_before_main_content','Fassbender_loadmore_parameters',40);
	function Fassbender_loadmore_parameters() {
	...
	<script type="text/javascript">
		var loadmore = {
			...
			"no_more_products":"<?php echo esc_html_e('No More Products','Fassbender-core'); ?>",
...
## Quick View Scripts
*************************************************/
if(get_theme_mod('Fassbender_paginate_type') == 'loadmore' || get_theme_mod('Fassbender_paginate_type') == 'infinite' || Fassbender_ft() == 'load-more' || Fassbender_ft() == 'infinite'){
	if(get_theme_mod('Fassbender_ajax_on_shop',0) == '1'){
		function Fassbender_load_more_scripts() {
			if(is_shop() || is_product_category()){
				if(get_theme_mod('Fassbender_paginate_type') == 'infinite' || Fassbender_ft() == 'infinite'){
					wp_enqueue_script( 'Fassbender-load-more',  plugins_url( 'js/infinite-scroll.js', __FILE__ ), true );
				} elseif(get_theme_mod('Fassbender_paginate_type') == 'loadmore' || Fassbender_ft() == 'load-more') {
					wp_enqueue_script( 'Fassbender-load-more',  plugins_url( 'js/load_more.js', __FILE__ ), true );
				}
			}
		}
		...
		function Fassbender_load_more_scripts() {
			if(is_shop() || is_product_category()){
				...
				wp_localize_script( 'Fassbender-load-more', 'loadmore', array(
					...
					'no_more_products' => esc_html__('No More Products','Fassbender-core'),
				));
			}
		}
	}
	add_action( 'wp_enqueue_scripts', 'Fassbender_load_more_scripts' );
}
...
add_action( 'wp_ajax_nopriv_load_more', 'Fassbender_load_more_callback' );
add_action( 'wp_ajax_load_more', 'Fassbender_load_more_callback' );
function Fassbender_load_more_callback() {
