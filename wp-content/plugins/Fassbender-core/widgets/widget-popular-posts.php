...
class widget_popular_posts extends WP_Widget { 
	...
	function __construct() {
		$widget_ops = array('description' => esc_html__('Display the popular posts','Fassbender-core') );
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'popular_posts' );
		 parent::__construct( 'popular_posts', esc_html__('Fassbender Popular Posts','Fassbender-core'), $widget_ops, $control_ops );
	}
		...
		<ul class="recent-posts widget_recent_post">
			...
			<?php $popularpost = new WP_Query( array( 
						'posts_per_page' => $number,
						 'meta_key' => 'Fassbender_post_views_count',
			...
			while ( $popularpost->have_posts() ) : $popularpost->the_post(); ?>
				...
				<li>
					<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
						...
						$imgresize = Fassbender_resize( $image_src, 80, 65, true, true, true );   
						?>
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of items to show:','Fassbender-core'); ?></label>
